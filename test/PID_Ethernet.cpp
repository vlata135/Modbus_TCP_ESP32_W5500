    ...
    while(1)
    {
        if(flag_run == 1)
        {
            ...
            if(u < 0)
            {
                ...
                shiftOut(DS,SHcp,MSBFIRST,M3_FW);
            ...
            {
                digitalWrite(STcp,LOW); //ground ST_CP and hold low for as long as you are transmitting
                shiftOut(DS,SHcp,MSBFIRST,M3_BW);
                digitalWrite(STcp,HIGH); //pull the ST_CPST_CP to save the data
                analogWrite(M3_PWM, pwr);
            }
...
{
    uint16_t u16vel = 0;
    u16vel = HexToDecimal(buffer[9])*1000 + HexToDecimal(buffer[10])*100 +
             HexToDecimal(buffer[11])*10  + HexToDecimal(buffer[12]); // 9 to 12 is index of position in data frame
    return u16vel;
